...
<body>
    ...
    <div id="roleDategrid" class="mini-datagrid" style="width:100%;height:100%;" allowResize="true"
        url="${ctx}/admin/merRole/listAll"  idField="role_id" multiSelect="true" >
    ...
    <script type="text/javascript">
        ...
        function edit(flag) {
        	if("f_insert" == flag ){
                ...
                if (row) {
                    ...
                    form.loading();
                    $.ajax({
                        url: '${ctx}/admin/merRole/' + row.role_id,
        ...
        function remove() {   
        	 ...
        	 if(row){
        		//删除之前验证所选记录是否存在
     			$.post('${ctx}/admin/merRole/delCheckExits/' + row.role_id,function(checkmsg){
     				if(checkmsg){
     					 if (confirm("确定删除选中记录？")) {
     						$.post('${ctx}/admin/merRole/roleCheckExits/' + row.role_id,function(rolCheckmsg){
 								if(rolCheckmsg){
 										//删除
 											$.post('${ctx}/admin/merRole/delete/' + row.role_id, function(data) {
 												if (data) {
 													notify('记录删除成功!');
 													 grid.reload();
 												} else {
        ...
        function saveRole(flag) {     
        	 ...
        	 var form;
             if("f_insert" == flag ){
             	url = "${ctx}/admin/merRole/create";
             	form = new mini.Form("#addform");
             }else if("f_edit" == flag ){
             	url = "${ctx}/admin/merRole/update";
        ...
        function bandResource() {  
        	var row = grid.getSelected();
            if (row) {
            	//funOptTree.loadData(null,'${ctx}/admin/merRole/allOpt/'+selected.role_id);
            	 $.ajax({
                     url: '${ctx}/admin/merRole/allOpt/' + row.role_id,
                     dataType:"json",
                     type:"get", 
                     contentType:'application/json;charset=UTF-8',  
                     success: function (text) {
        ...
        function  doBandResource(){
        	...
        	if(value.length>0){
				$.post(
						'${ctx}/admin/merRole/customer_bund/'
								+ row.role_id + "/"
								+ value,
						function(data) {
							if (data) {
        ...
        function onRoleNmValidation(e) {
            if (e.isValid) {
            	...
            	if(roleNm != e.value){
            		$.ajax({
                        url: '${ctx}/admin/merRole/checkRoleNm',
                        async : false, 
