...
<body>
    ...
    <div id="userDategrid" class="mini-datagrid" style="width:100%;height:100%;" allowResize="true"
        url="${ctx}/sys/userGroup/list"  idField="grp_id" multiSelect="true" >
    ...
    <script type="text/javascript">
        ...
        function edit() {
             ...
             if (row) {
                 ...
                 form.loading();
                 $.ajax({
                     url: '${ctx}/sys/userGroup/' + row.grp_id,
       ...
       /*  $("#search_like_loginId","#search_like_userName").keydown(function(){
        ...
        function remove() {   
	       	 ...
	       	 if(row){
	    		if (confirm("确定删除选中记录？")) {
	    			$.post('${ctx}/sys/userGroup/delete/' + row.grp_id,
							function(data) {
								if (data) {
									notify('记录删除成功!');
									 grid.reload();
        ...
        function getUserInfo(){
        	...
        	var row = grid.getSelected();
            if (row) {
            	$.post('${ctx}/sys/userGroup/checkExits/' + row.grp_id,
						function(result) {
							if (!result) {
								$.post('${ctx}/sys/userGroup/getUserNameBygrp_id/' + row.grp_id,
										function(result) {
        ...
        function saveGroup(flag) {            
       	 ...
       	 var form;
            if("f_insert" == flag ){
            	url = "${ctx}/sys/userGroup/create";
            	form = new mini.Form("#addform");
            }else if("f_edit" == flag ){
            	url = "${ctx}/sys/userGroup/update";
        ...
        function bandUserInfo() {  
        	var row = grid.getSelected();
            if (row) {
            	var url = "${ctx}/sys/userGroup/getAllUser/";
            	 $.ajax({
                     url: url + row.grp_id,
                     dataType:"json",
                     type:"get", 
        ...
        function  doBandUserInfo(){
        	var row = grid.getSelected();
        	var url = "${ctx}/sys/userGroup/insertUesrGroupUser/";
        ...
        function bandRoleInfo() {  
        	var row = grid.getSelected();
            if (row) {
            	var url  = "${ctx}/sys/userGroup/getAllRole/";
            	 $.ajax({
                     url: url + row.grp_id,
                     dataType:"json",
                     type:"get", 
        ...
        function  doBandRoleInfo(){
        	...
        	var url =  "${ctx}/sys/userGroup/insertUesrGroupRole/";
