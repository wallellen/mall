 ...
 */
@Controller
@RequestMapping("/admin/merUser")
public class MerUserController {
	@RequestMapping()
	public String init() {
		return "admin/merRbac/merUser";
	}
	...
	public String toAdd(){
		return "admin/merRbac/merUser_edit";
	}
	...
	public boolean save(@RequestBody final SysUser user){
		try{
			if(null != user.getUser_id() && user.getUser_id() > 0 ){
				...
				user.setUser_type( SystemConstant.UserType.MERCHANT_USER);
				D.insert(user);
			}
...
//		List<UserGroup> userGroupList = userGroupService.selectByExample(null);
	...
	public List<Map<String, Object>> getAllRole(
		...
		// 1、查询出所有角色
		List<SysRole> roleList = D.sql("select * from T_P_ROLE where user_type = ? ").many(SysRole.class,SystemConstant.UserType.MERCHANT_USER);
		// 2、找出与用户角色关联的信息
