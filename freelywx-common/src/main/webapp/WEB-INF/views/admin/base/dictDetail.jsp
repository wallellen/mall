...
<body>
	...
	<div class="mini-fit">
		<div id="dictDataGrid" class="mini-datagrid" style="width: 100%; height: 100%;" allowResize="true"
			url="${ctx}/admin/dictDetail/list?dictId=${dictId}" idField="dict_detail_id" multiSelect="true">
	...
	<script type="text/javascript">
		...
		function edit(flag) {
			if ("f_insert" == flag) {
				...
				if (row) {
					...
					form.loading();
					$.ajax({
						url : '${ctx}/admin/dictDetail/' + row.dict_id + "/" + row.dict_param_value + "/" + row.dict_param_name,
		...
		function remove() {
			...
			if (row) {
				if (confirm("确定删除字典？")) {
					//删除选中记录
					$.post("${ctx}/admin/dictDetail/delete/" + row.dict_id + "/"
		...
		function save(flag) {
			...
			if ("f_insert" == flag) {
				...
				url = "${ctx}/admin/dictDetail/create";
			} else if ("f_edit" == flag) {
				form = new mini.Form("#editform");
				url = "${ctx}/admin/dictDetail/update";
			}
			form.validate();
