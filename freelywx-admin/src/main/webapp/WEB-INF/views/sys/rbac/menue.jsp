...
<body>
    ...
    <div class="mini-fit" >
    <div id="optTreeGrid" class="mini-treegrid" style="width:100%;height:100%;"  url="${ctx}/sys/menue/listAll" showTreeIcon="true" treeColumn="menue_nm" idField="menue_id" parentField="par_menue_id" resultAsTree="false"  allowResize="true" expandOnLoad="true">
    ...
    <script type="text/javascript">
        ...
        function remove() {   
        	 ...
        	 if(row){
        		//删除之前验证所选记录是否存在
        		$.post("${ctx}/sys/menue/fredelcheck/"+ row.menue_id,function(msg) {
        			if (msg) {
						$.post("${ctx}/sys/menue/checkChild/"+ row.menue_id,function(checkMsg) {
							if (checkMsg) {
								if (confirm("确定删除菜单？")) {
									//删除选中记录
									$.post("${ctx}/sys/menue/deleteMenue/"+ row.menue_id,function(result) {
										if (result) {
											notify('删除成功');
											optTreeGrid.reload();
										} else {
