...
@Controller
@RequestMapping(value = "/sys/funopt")
public class FunOptController {
	...
	public void listAll(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		List<SysFunOpt> allFunOpt = D.sql("Select * from t_sys_fun_opt  ").many(SysFunOpt.class );
	...
	public boolean save(@RequestBody SysFunOpt funopt) {
		if(funopt.getFun_opt_id()  != null  ){
			...
			funopt.setUser_type(SystemConstant.UserType.SYSTEM_USER);
			D.insert(funopt);
		}
	...
	public boolean check(String fun_opt_nm,String fun_opt_id) {
		...
		List<SysFunOpt> foList = D.sql("select * from t_sys_fun_opt where fun_opt_nm = ?").many(SysFunOpt.class, fun_opt_nm);
	...
	public boolean checkUrl(String url,String fun_opt_id) { 
		...
		List<SysFunOpt> foList = D.sql("select * from t_sys_fun_opt where url = ?").many(SysFunOpt.class, url);
		return foList.size() > 0 ? true : false;
//		String regEx = "^[A-Za-z]+$";
...
//		return false;
	...
	public boolean checkChild(@PathVariable("funOptId") Long funOptId) {
		List<SysFunOpt> list = D.sql("select * from t_sys_fun_opt where parent_fun_opt_id = ?").many(SysFunOpt.class, funOptId);
		return list.isEmpty();
	}
	...
	public List<Map<String, Object>> selFunopt() {
		...
		List<SysFunOpt> list = D.sql("select * from t_sys_fun_opt where parent_fun_opt_id is not null   ").many(SysFunOpt.class ) ;
