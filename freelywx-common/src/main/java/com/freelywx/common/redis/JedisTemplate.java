package com.freelywx.common.redis;
...
public class JedisTemplate {
    ...
    public void setObj(final String key,final byte[] obj) {
    	execute(new JedisActionNoResult() {
            @Override
            public void action(Jedis jedis) {
    ...
    public void delObj(final String key ) {
    	execute(new JedisActionNoResult() {
            @Override
            public void action(Jedis jedis) {
 ...
 // ////////////// 关于List ///////////////////////////
    public void lObjPush(final String key, final byte[]... values) {
        execute(new JedisActionNoResult() {
            @Override
            public void action(Jedis jedis) {
    ...
    public List<byte[]> lObjRange(final String key, final Long start, final Long end) {
    ...
    public byte[] rObjPop(final String key) {
        return execute(new JedisAction<byte[]>() {
     ...
     * 返回List长度, key不存在时返回0，key类型不是list时抛出异常.
     */
    public Long lObjLen(final String key) {
        return execute(new JedisAction<Long>() {
     ...
     * 删除List中的第一个等于value的元素，value不存在或key不存在时返回false.
     */
    public Boolean lobjRemOne(final String key, final byte[] value) {
        return execute(new JedisAction<Boolean>() {
            @Override
            public Boolean action(Jedis jedis) {
                Long count = jedis.lrem(key.getBytes(), 1, value);
                return (count == 1);
            }
        });
    }
     ...
     * 删除List中的所有等于value的元素，value不存在或key不存在时返回false.
     */
    public Boolean lobjRemAll(final String key, final byte[] value) {
        return execute(new JedisAction<Boolean>() {
            @Override
            public Boolean action(Jedis jedis) {
                Long count = jedis.lrem(key.getBytes(), 0, value);
                return (count > 0);
            }
        });
    }
