	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.funFilterEligibleFile = function(files){
				...
				var arrType = ["png","jpg","jpeg"];
				var maxSize = 2 * 1024 * 1024;// 文件最大（单位：b）
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size > maxSize) {
						...
						for ( var j = 0; j < arrType.length; j++) {
							if(file.type.search(arrType[j]) != -1)
								boo = true;
						}
						if(boo){
							arrFiles.push(file);	
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '<input type="hidden" id="imgfile'+file.index+'" name="imgfile" value=""/>';
				...
				//	html += '			<p class="file_name">' + file.name + '</p>';
					...
					html += '		</div>';
				//	html += '          <div class="delect file_del"></div>  ';
				...
				//	html += '			<p class="file_name">' + file.name + '</p>';
					...
					html += '	</div>';
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + fileImgSrc + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                    html += '		</div>';
					html += '	<p id="uploadProgress_'+file.index+'" class="file_progress"></p>';
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">上传失败，请重试</p>';
					html += '	<p id="uploadSuccess_'+file.index+'" class="file_success"></p>';
					html += '</div>';
				...
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						if(allFiles.length > 0){
							var fileObj = document.getElementById("fileImage").files[0]; 
							var FileController = "http://"+serverurl+"/wxmall/tribe/upload1"; 
							//var FileController = "http://127.0.0.1:8080/wxmall/tribe/upload1"; 
							var form = new FormData();
							form.append("fileList", fileObj);
							var xhr = new XMLHttpRequest();
							xhr.open("post", FileController, true);
							xhr.onreadystatechange = function () {
								if(xhr.readyState == 4 && xhr.status == 200){
									$('#imgfile' + (allFiles.length - 1)).val(xhr.responseText)
									//alert($('#imgfile' + (allFiles.length - 1)).val())
								}
							};
							xhr.onload = function () { };
							xhr.send(form);
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								var aa=$('.file_del[index!="sc"]').length;
								if(aa==4){
