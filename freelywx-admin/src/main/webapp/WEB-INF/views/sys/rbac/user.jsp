...
<body>
    ...
    <div id="userDategrid" class="mini-datagrid" style="width:100%;height:100%;" allowResize="true"
        url="${ctx}/sys/user/list"  idField="user_id" multiSelect="true" >
    ...
    <script type="text/javascript">
        ...
        function bandUserInfo() {  
        	var row = grid.getSelected();
            if (row) {
            	var url  = "${ctx}/sys/user/getAllUserGroup/";
            	 $.ajax({
                     url: url + row.user_id,
                     dataType:"json",
                     type:"get", 
        ...
        function  doBandUserInfo(){
        	...
        	var url = "${ctx}/sys/user/insertUesrGroupUser/";
        ...
        function bandRoleInfo() {  
            ...
            if (row) {
            	 $.ajax({
                     url: "${ctx}/sys/user/getAllRole/" + row.user_id,
                     dataType:"json",
                     type:"get", 
                     contentType:'application/json;charset=UTF-8',  
                     success: function (text) {
        ...
        function  doBandRoleInfo(){
        	...
        	var url  = "${ctx}/sys/user/insertUesrRole/";
        	$.post(url + row.user_id, {'boundInfos' : value}, function(data) {
				if (data) {
					roleBindWindow.hide(); 
					notify("保存成功");
