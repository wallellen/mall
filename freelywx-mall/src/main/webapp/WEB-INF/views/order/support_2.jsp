...
<title>
	<c:if test="${pay_type == '1' }">众筹赞助</c:if>
	<c:if test="${pay_type != '1' }">补齐金额</c:if>
	</title>
...
<body class="body-bj">
	...
	<div class="my-page-mondy">
		 <c:if test="${pay_type == '1' }">
    		<p>输入赞助金额<span class="red_center">愿望目标还差：<fmt:formatNumber value="${needPayAmount/100}" type="currency" pattern="￥0.00"/>元</span></p>
         </c:if>  
   	 	<c:if test="${pay_type == '2' }">
   	 		<p>需支付剩余金额<span ><fmt:formatNumber value="${needPayAmount/100}" type="currency" pattern="￥0.00"/>元</span></p>	
    	</c:if>  
   
	    <c:if test="${pay_type == '1' }">
	     <div class="input-box">
	    	<div class="box-boder">
	       		 <input type="text" id="treatPay" />
	       		 <input type="hidden" id="payFee" />
	        </div>
	        <div class="message">
	        	<div class="ly-title">留言</div>
	        	<textarea class="ly-textarea" id="message" placeholder="我是爱你的！你需要我，我就来了！" ></textarea>
	        </div>
	     </div> 
	   </div> 
    ...
    <div class="press">
	    <a href="javascript:void(0);" id="wx" class="bj-blue">微信支付</a>
	</div> 
    <div class="tips">
    	<p>说明：</p>
        <p>1、为了便于您查询，请关注微信号：<strong>锋付智能</strong>；</p>
		<p>2、如果未成功积满金额，建议发起者补齐金额；</p>
		<p>3、<strong>如果退款，款项会全部退至发起者账户。</strong></p>
