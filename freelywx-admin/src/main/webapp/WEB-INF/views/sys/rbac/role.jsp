...
<body>
    ...
    <div id="roleDategrid" class="mini-datagrid" style="width:100%;height:100%;" allowResize="true"
        url="${ctx}/sys/role/listAll"  idField="role_id" multiSelect="true" >
    ...
    <script type="text/javascript">
        ...
        function remove() {   
        	 ...
        	 if(row){
        		//删除之前验证所选记录是否存在
     			$.post('${ctx}/sys/role/delCheckExits/' + row.role_id,function(checkmsg){
     				if(checkmsg){
     					 if (confirm("确定删除选中记录？")) {
     						$.post('${ctx}/sys/role/roleCheckExits/' + row.role_id,function(rolCheckmsg){
 								if(rolCheckmsg){
 										//删除
 											$.post('${ctx}/sys/role/delete/' + row.role_id, function(data) {
 												if (data) {
 													notify('记录删除成功!');
 													 grid.reload();
 												} else {
        ...
        function bandResource() {  
        	var row = grid.getSelected();
            if (row) {
            	//funOptTree.loadData(null,'${ctx}/sys/role/allOpt/'+selected.role_id);
            	 $.ajax({
                     url: '${ctx}/sys/role/allOpt/' + row.role_id,
                     dataType:"json",
                     type:"get", 
                     contentType:'application/json;charset=UTF-8',  
                     success: function (text) {
